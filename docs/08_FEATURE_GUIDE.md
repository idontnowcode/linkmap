...
> 최종 갱신: 2026-09-18 · 대상 버전: `main` (UX 개선 13개 항목 일괄 적용 — 폴더 태그 계층/병합, 전문검색 확장, 최근 연 링크, 깨진 링크 검사, 데이터 내보내기/가져오기, 커맨드 팔레트, 빠른 캡처 팝업 등. 이전 버전 최신화 이력은 `docs/11_VERSION_UPGRADE_PLAN.md` 참조)
...
- **스마트 뷰**: 모든 링크 · 즐겨찾기 · 최근 추가(7일 이내) · **최근 연 링크**(7일 이내 `openTarget`으로 실제로 열어본 것, 2026-09-18 추가 — "최근 추가"=생성일과 달리 `links.opened_at` 기준, 최근에 연 순서로 정렬됨) · 휴지통 — 각각 실시간 카운트 표시
...
- **태그 섹션**
  ...
  - 섹션 전체 접기 가능.
  - 우클릭 → **이름·색상 편집**(`TagFormDialog`를 수정 모드로 오픈, 2026-09-17 추가) / **다른 태그와 병합**(`TagMergeDialog`, 2026-09-18 추가 — 대상 태그를 검색해 선택하면 원본 태그의 모든 링크 연결이 대상으로 옮겨지고 원본은 삭제됨, 되돌릴 수 없음) / 태그 삭제.
...
- **외부 파일 변경 감지(2026-09-18)** — `kind:'file'` 링크는 저장 시점의 파일 mtime(`links.file_mtime`)을 함께 기록한다. 상세 패널이 열려 있을 때 실제 파일의 현재 mtime(`path:mtime` IPC)과 비교해 2초 이상 차이나면 경로 아래에 "⚠ 파일이 변경됨" 배지를, 파일을 찾을 수 없으면 "⚠ 파일 없음" 배지를 표시한다. 대량 IPC 호출을 피하려고 목록 카드마다는 확인하지 않고 상세 패널에 선택된 링크 하나만 확인한다.
- **즐겨찾기 토글**.
...
- **인용 추가(2026-09-18)** — 툴바의 "인용 추가" 토글을 열면 페이지/위치(선택)와 인용 내용 입력란이 나타난다. "메모에 추가"를 누르면 메모 끝에 `> p.N: 텍스트` 형식으로 append되고 즉시 저장된다. 미리보기 중인 파일(PDF 등)의 특정 부분을 정교한 좌표 기반 하이라이트 없이 실용적인 수준으로 인용해두는 용도.
...
- **중복 웹 링크 경고(2026-09-18)** — 새 링크 추가 시(편집 아님) 종류가 웹이고 이미 같은 URL의 활성 링크가 있으면 URL 입력란 아래 경고가 표시된다("이미 저장된 링크입니다 — '제목'"). 파일 링크처럼 자동으로 재사용하지는 않고 저장 자체는 막지 않는다 — 같은 URL을 다른 맥락으로 또 저장하고 싶을 수 있어 최종 판단은 사용자에게 맡긴다.
...
**깨진 링크 확인(2026-09-18)** — 설정 화면 하단 "깨진 링크 확인" 버튼(수동, 자동/주기 검사는 없음). 모든 활성 웹 링크에 HEAD 요청(막혀 있으면 GET으로 재시도)을 동시 5개 제한으로 보내 `links.link_checked_at`/`links.link_broken`에 결과를 기록한다. 응답 없는 링크는 링크 카드 제목 앞과 상세 정보 탭에 "⚠ 응답 없는 링크" 배지로 표시된다.

**데이터 내보내기 / 가져오기(2026-09-18)** — 설정 화면 하단 "내보내기(JSON)"/"가져오기(JSON)" 버튼. 내보내기는 `graphRepo.snapshot()`과 동일한 형태(links/tags/relations/collections/linkTags/collectionLinks + 폴더 동기화 제외 목록)를 JSON 하나로 저장(`dialog.showSaveDialog`). 가져오기는 JSON 파일을 선택해(`dialog.showOpenDialog`) 현재 DB에 **추가만** 한다 — 기존 데이터를 지우거나 덮어쓰지 않고, 이미 같은 id가 있으면 건너뛴다(`onConflictDoNothing`). 컬렉션은 `parentId` 위상정렬로 부모를 먼저 넣어 FK 오류를 피한다. 클라우드 동기화가 아니라 파일 단위 로컬 백업/이전용.
...
- "N개 가져오기" → 선택된 파일이 `kind: 'file'` 링크로 생성되고, **경로상의 모든 조상 폴더 각각에 대응하는 태그 전부**(2026-09-18부터 계층화)가 부여된다.
- **확장자별 보조 태그(2026-09-18)** — 파일 트리 위 "확장자별 보조 태그 자동 추가" 체크박스(기본 OFF). 켜면 확장자 이름 그대로의 일반 태그(`pdf`, `xlsx`, `png` 등 — `sourcePath` 없음, "태그" 섹션에 표시)를 만들어 함께 부여한다. 같은 이름의 태그가 이미 있으면(수동 생성분 포함) 재사용한다.
...
**하위 폴더 태그 계층 (2026-09-18)** — 예: `Datasheets/Old/v1.pdf`를 가져오면 그 파일에는 `Datasheets` 태그와 `Datasheets/Old` 태그가 **둘 다** 부여된다(이름에 "/"를 써서 좌측 레일 태그 트리·폴더 트리에서 자동으로 계층처럼 보임 — 3장 참조). 하위 폴더마다 태그가 없으면 새로 생성하며, **모든 레벨(루트뿐 아니라 하위 폴더도)이 각자 `source_path`를 가져** 하위 폴더 태그만 따로 동기화(6.13절)할 수도 있다. 폴더가 사라져 하위 태그가 비어도 자동 삭제되지 않는다(수동 삭제).
...
**동기화 범위 편집 (2026-09-18)** — 폴더 태그 hover 시 🔄 왼쪽에 나타나는 필터 아이콘(`ListFilter`) → `FolderSyncExclusionDialog` 오픈. `FolderImportDialog`와 같은 트리+체크박스 UI를 재사용하되 이번엔 현재 제외 상태를 반영해 미리 체크되어 있다. 체크 해제한 파일·폴더는 gitignore 스타일로 `folder_sync_exclusions(tag_id, relative_path)` 테이블에 최소 경로 집합으로 저장되고, **저장만으로는 아무것도 즉시 반영되지 않는다** — 다음 🔄 동기화 미리보기부터 제외 범위가 적용되어(이미 있던 파일은 "더 이상 없음"으로 표시) 사용자가 확인 후 "완료"를 눌러야 실제로 휴지통行이 반영된다.

---

### 6.14 커맨드 팔레트 (2026-09-18)

`Ctrl/Cmd+K`로 어디서든 열리는 오버레이(`CommandPalette.tsx`). 입력창에 타이핑하면 링크 제목·태그 이름을 대상으로 자체 구현한 경량 퍼지 매칭(`lib/fuzzyMatch.ts` — 부분문자열 우선, 없으면 subsequence 매칭 점수화, 외부 라이브러리 없음)으로 실시간 필터링한다. `↑/↓`로 이동, `Enter`로 선택, `Esc`로 닫기. 링크를 선택하면 해당 노드를 선택+포커스(그래프 패널이 있다면 자동 센터링), 태그를 선택하면 그 태그 뷰로 전환한다. 쿼리가 비어 있으면 최근 생성된 링크 몇 개를 기본으로 보여준다.

### 6.15 전역 단축키 빠른 캡처 팝업 (2026-09-18)

앱이 백그라운드에 있어도 `Ctrl/Cmd+Shift+L`(Electron `globalShortcut`)을 누르면 작은 always-on-top 프레임리스 팝업 창(`QuickCapturePopup.tsx`)이 뜬다.
- 별도 렌더러 번들 없이 같은 `index.html`을 `#popup` 해시로 로드하고(`main.tsx`가 해시를 보고 `<QuickCapturePopup/>`을 렌더링), 메인 창과는 **독립된 렌더러 인스턴스**(별도 zustand 스토어)라 `window.api`를 직접 호출한다.
- URL 붙여넣기(자동 메타 수집 후 웹 링크로 저장) 또는 파일 드롭(파일 링크로 저장)을 지원. `Enter`로 저장, `Esc` 또는 포커스를 잃으면(`blur`) 자동으로 닫힘.
- 팝업에서 만든 링크는 메인 창이 즉시 알 수 없으므로, `links:create` 처리 후 메인 프로세스가 모든 창에 `graph:changed` 이벤트를 브로드캐스트(`windowEvents.ts`)하고 메인 창의 `App.tsx`가 이를 구독해 스냅샷을 자동 새로고침한다.
...
12. **UX 개선 일괄 적용 (2026-09-18)**: 유사 앱(Raindrop.io/Eagle/TagSpaces/Linkwarden/Karakeep/Obsidian) 벤치마킹 기반으로 승인된 13개 항목을 순서대로 구현 — (A) 하위 폴더 태그 계층화 + 동기화 제외 범위 편집 + 태그 병합, (B) PDF/DOCX/XLSX 전문검색 확장(`unpdf`) + 외부 파일 변경(mtime) 감지, (C) 중복 웹 링크 경고 + 확장자 자동 태그 + "최근 연 링크" 스마트뷰 + 깨진 링크 수동 검사 + 전체 데이터 JSON 내보내기/가져오기, (D) 커맨드 팔레트(Ctrl+K) + 전역 단축키 빠른 캡처 팝업(Ctrl+Shift+L) + 메모 인용 추가. 웹페이지 스냅샷/아카이빙·AI 자동 태깅·시스템 트레이 상주는 의도적으로 범위 밖(스코프 제외).
...
- **전체 데이터 JSON 내보내기/가져오기는 지원**(6.8절)하지만, Chrome/Raindrop 등 **외부 북마크 마이그레이션 포맷은 지원하지 않음**(LinkMap 자체 포맷만).
- **동기화/멀티디바이스 없음** — 완전 로컬 단일 기기용.
- **AI 관계 추천은 로컬 휴리스틱**이며 실제 LLM 기반 의미 분석은 아님.
- **깨진 링크 검사는 수동/일회성**(설정 화면 버튼) — 주기적 자동 검사는 없음.
