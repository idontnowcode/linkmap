...
> 최종 갱신: 2026-09-17 · 대상 버전: `main` (버전 최신화 — 파일 미리보기 강화/폴더 가져오기·동기화/태그 계층 추가 + UI 정리·버그 수정 + 폴더 가져오기·동기화 트리/미리보기 개편 기준, `docs/11_VERSION_UPGRADE_PLAN.md` 참조)
...
│        │  │  ├─ FolderImportDialog.tsx      # 폴더 가져오기 다이얼로그(트리 UI)
│        │  │  └─ FolderSyncPreviewDialog.tsx # 폴더 동기화 New/Deleted 미리보기 다이얼로그
...
| 폴더 가져오기/동기화 | `folderList`(재귀 스캔, 파일+폴더+depth), `folderImport`(선택 파일 일괄 링크화), `folderSyncPreview`(변경분 미리보기·미반영), `folderSync`(실제 반영) |
...
- **태그 섹션**
  ...
  - **색상**: `TAG_PALETTE`(8색, `src/shared/types.ts`)를 기존 태그 개수만큼 순환시켜 새 태그 기본색으로 배정(2026-09-17) — 이전엔 항상 팔레트 첫 색(파랑)으로 시작해 태그가 전부 파랑이 되던 문제가 있었음. 색은 수동으로도 고를 수 있음.
  - **태그 계층 표현**: 이름에 `"/"`가 있고 그 상위 경로가 실제로 존재하는 태그 이름이면(예: `Project: X`가 있어야 `Project: X/Firmware`가 자식) 들여쓰기 + ▸/▾ 접기로 트리처럼 표시(6.12절, DB는 평면 그대로).
  - 섹션 전체 접기 가능.
  - 우클릭 → **이름·색상 편집**(`TagFormDialog`를 수정 모드로 오픈, 2026-09-17 추가) / 태그 삭제.
  - **링크를 태그 위로 드래그하면 그 태그가 부여됨** (다중 선택 상태에서 드래그하면 선택된 전체에 일괄 부여).
- **폴더 섹션** (태그 섹션 위, 2026-09-17 분리 — 아이콘만으로는 구분이 부족하다는 피드백)
  - 폴더 가져오기로 생성된 태그(`sourcePath` 보유)만 모아서 보여준다. 데이터상으로는 여전히 그냥 태그(`tags` 테이블 그대로)이고, 화면에서만 두 그룹으로 나눔.
  - 행 모양·동작은 태그 섹션과 동일(클릭 필터링, 드래그로 부여, 우클릭 편집/삭제) + hover 시 🔄(동기화 미리보기) 아이콘.
  - 섹션 제목 옆 `+`는 `FolderImportDialog` 오픈(폴더 가져오기와 동일).
...
- 선택한 폴더 아래 모든 파일·폴더를 재귀 스캔(`.git`/`node_modules`/`.svn`/`.hg`/`__pycache__`/점폴더는 자동 제외, 최대 3000개, 넘으면 "일부만 표시" 안내), **트리 구조**로 표시(2026-09-17 개편, 이전엔 평면 목록).
  - 루트 기준 3단계까지는 기본 펼침, 그 아래(depth 4+)는 기본 접힘 — 폴더명 옆 화살표로 펼치기/접기.
  - **기본적으로 전체 파일이 선택된 상태**(옵트아웃 방식) — 체크 해제한 파일만 가져오기에서 제외된다.
  - 폴더 체크박스는 하위 파일 전체를 한 번에 켜고 끄는 삼중상태(전체/일부/없음, `indeterminate`) 토글.
- "N개 가져오기" → 선택된 파일이 `kind: 'file'` 링크로 생성되고, **폴더 이름을 딴 태그**(`tags.source_path` = 폴더 절대경로)가 모든 선택 파일에 부여된다.
...
6.12절로 만든 폴더 태그(`source_path` 보유)에 한해, 원본 폴더를 다시 스캔해 변경 사항을 반영한다. **2026-09-17부터 미리보기 확인 단계가 추가됨** — 동기화 버튼을 눌러도 바로 반영되지 않고, 무엇이 바뀌는지 먼저 보여준 뒤 "완료"를 눌러야 실제로 적용된다.
- 사이드바 태그 목록에서 폴더 태그에 hover하면 나타나는 🔄 아이콘 클릭 → `FolderSyncPreviewDialog` 오픈(`folder:syncPreview` IPC — 아무것도 반영하지 않고 재스캔·비교만 수행).
- **New(새로 추가됨)**: 초록 배지로 강조 표시된 목록. **Deleted(더 이상 없음)**: 취소선 + 회색으로 표시된 목록(=적용 시 휴지통行). 변화 없는 항목 수는 하단에 텍스트로만 요약.
- 변경 사항이 없으면 "완료" 버튼이 "변경 사항 없음"으로 비활성화.
- **"완료 — 최신 버전으로 반영"**을 눌러야 실제 반영(`folder:sync` IPC) — 새 파일은 링크 생성, 사라진 파일의 링크는 **휴지통으로 이동**(영구 삭제 아님). "취소"를 누르면 아무 변화 없이 닫힘.
...
11. **UI 정리 + 버그 다잡기 (2026-09-15~17)**: 사용자가 태그/컬렉션 이중 관리에 시간을 쓰기 어렵다고 해 그래프·컬렉션 UI를 숨김(코드는 보존, 재활성화 가능) → 그 과정에서 드러난 회귀(고급 검색·저장된 필터 진입점 소실) 수정 → 관계 탭 드래그드롭 확장, 파일 미리보기 78vh 확대, PDF `ERR_BLOCKED_BY_CSP` 크래시 원인 규명 후 CSP 메타 태그 제거 → **폴더 가져오기/동기화를 트리 UI + 옵트아웃 전체선택 + "동기화 전 New/Deleted 미리보기 확인" 흐름으로 개편**(6.12~6.13절).
